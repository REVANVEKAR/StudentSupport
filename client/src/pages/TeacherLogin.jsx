  ...
  useEffect(() => {
    ...
    if (isSuccess || user) {
      if (user.role === 'teacher' || user.role === 'admin') {
        ...
        toast.error('This login is only for teachers');
        dispatch(reset());
      }
    }
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
        <div>
          <h1 className="text-3xl font-bold text-center text-gray-900 flex items-center justify-center">
            <FaSignInAlt className="mr-2" /> Teacher Sign in
          </h1>
          <p className="mt-2 text-center text-gray-600">Sign in to your teacher account</p>
        </div>
